...
define(function(require){
    ...
    var VCanvas = (function(){
        ...
        function drawDisc(ctx, opt){
            ...
            var arr = [];
            var oRadius = opt.radius;    //外半径
            var iRadius = oRadius-opt.lineLength;  //内半径
            for(var i=0; i<len; i++){
                var obj = {};
                var deg = Math.PI*(1-i/(len-1));
        ...
        function repaintDisc(ctx, opt){
            ...
            // var pt = parseInt(time/lineArr.length);
            // var timerInterval = setInterval(function(){
            //     if(lineArr.length){
            //         drawLine(ctx, lineArr[0]);
            //         lineArr.shift();
            //     } else {
        ...
        return function(opt){
            opt = extend({
                canvas: 'speed',    //canvas id
                nColor: ['#bd7845', '#922f2b'], //数字颜色
                lineWidth:1,    //线粗细
                lineLength: 15, //线长度
                lColor: ['#ec904c', '#d5d5d5'], //线颜色
                lineNumber: 30,  //线个数,
                scale: 1,
                font: '12px Arial'
            }, opt);

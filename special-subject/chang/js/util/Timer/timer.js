...
//                console.log('进入run的逻辑');
  ...
  //              console.log('进入0的逻辑');
        ...
        prop.run = function(){
            var time = Math.floor( ( this.opt.endTime.getTime() - this.getNow().getTime() ) / 1000 );
            var times = {};
            if( time > 0 ){
                ...
                this.cb(1,times);
                 ...
                 this.cb(2,{"hours":"00", "minitues":"00", "seconds": "00"});  
            }
        }
...
//            console.log('status first',status);
    ...
    //        console.log('thiscount',this.count);
            var offset=tempDate.getTime()-(this.startTime.getTime()+this.count*1000);
...
//                console.log('status为0');
        ...
        prop.getStatus = function(){
            ...
            var start = this.opt.startTime.getTime();
            if(  !( now < start ) ) {
                var time = Math.floor( ( this.opt.endTime.getTime() - (new Date().getTime()+this.serverOffsetTime) ) / 1000 );
                var times = {};
                if( time > 0 ){
                    var hour = Math.floor( time / 3600 );
                    var min = Math.floor( time / 60 ) % 60 ;
                    ...
                    return [2,{"hours":"00", "minitues":"00", "seconds": "00"}]; 
                }
            }else{
                return [0,undefined];
            }

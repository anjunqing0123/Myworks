define(function(require, exports){
	...
	function GroupController(lists, option){
		...
		var opt = _.extend(this, {
			activeClassName: 'now', //active class
			tabTagName: 'a', //tagName, for binding event
			tabMoreTagName: 'a', //more tagName, for binding event
			$container: $(), //jquery object, the container of visible buttons
			$containerMore: $(),//jquery object, the container of hidden buttons
			showCount: 4, //count of visible buttons
			/*
		...
		this.eventAggregator && this.eventAggregator.subscribe('openNextPage', function(callback, correction){
			var correction = correction || 0; //index修正，用于直播播放0点时重新读取当天数据
			//播放下一页第一集
			var index; //当前分组的index
	...
	GroupController.prototype = {
		init: function(options){
			 ...
			 */
			var option = $.extend({
				page: this.groups.length - 1 //page根据对象的page属性值决定
			}, options);

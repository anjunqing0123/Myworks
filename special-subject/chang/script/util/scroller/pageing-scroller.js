 ...
 * @Qq			175225632
 */
define("util/scroller/pageing-scroller", [ "core/jquery/1.8.3/jquery", "core/underscore/1.8.3/underscore", "../scroller/scroller", "../event/event-mouse-wheel", "./scroller.css" ], function(require, exports) {
    ...
    function PageingScroller(option) {
        // var $clone = $cont.find('ul').children().clone();
        // $cont.find('ul').append($clone);
        var opt = this.opt = option || {};
        var necessary = [ "$cont", "width", "count", "events", "pageSize" ];
    ...
    _.extend(PageingScroller.prototype, {
        initialize: function() {
            var opt = this.opt;
            var option = {
                wheelPixel: opt.width,
                maxWidth: opt.$cont.outerWidth(),
                horizontal: true,
                showScroller: false,
                animate: true,
                mouseWheel: false
            };
            var scroller = this.scroller = opt.$cont.ppScroller(option).scroll();
            var pager = new Pager({

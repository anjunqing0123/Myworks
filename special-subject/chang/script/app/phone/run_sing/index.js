...
define("app/phone/run_sing/run_sing", [ "core/jquery/1.8.3/jquery", "util/others/others", "util/cookie/cookie", "core/underscore/1.8.3/underscore", "util/swipe/swiper.min", "util/vote/vote", "util/loader/loader", "util/log/log", "util/platform/plt", "util/browser/browser", "util/net/urlquery", "util/user/user", "client", "util/Timer/timer", "util/vote/uniformDate", "util/ppsdk/sdkUtil", "util/log/alertBox", "util/Timer/timer", "util/vcanvas/vcanvas" ], function(require) {
    ...
    var update = function() {
        Loader.ajax({
            ...
            success: function(data) {
                ...
                upId = data.data.speedUpVoteId, downId = data.data.slowDownVoteId, upVote = data.data.speedUpVote.counter, 
                downVote = data.data.slowDownVote.counter, countDownTime = others.dateFormat(others.newDate(data.data.endedAt) - others.newDate(), "hh:mm:ss", true);
                ...
                clearTimer = new timer({
                    ...
                    callback: function(status, times) {
                        ...
                        if (nextUpdate - new Date().getTime() < -1e3 * 10 && nextUpdate !== 0) {
                            ...
                            Loader.ajax({
                                ...
                                success: function(data) {
                                    var data = data.data;
                                    $module.find(".num").text(data.speed).attr("next-update", data.offline_on * 1e3);
                                    console.log(new Date(data.offline_on * 1e3), "offline_on");
...
define("app/phone/run_sing/choujin", [ "core/zepto/zepto", "util/others/others", "core/jquery/1.8.3/jquery", "util/cookie/cookie", "core/underscore/1.8.3/underscore", "util/swipe/swiper.min", "util/vote/vote", "util/loader/loader", "util/log/log", "util/platform/plt", "util/browser/browser", "util/net/urlquery", "util/user/user", "client", "util/Timer/timer", "util/vote/uniformDate", "util/ppsdk/sdkUtil", "util/log/alertBox", "util/ppsdk/sdkUtil" ], function(require) {
    ...
    $module.find(".avartar a").click(function() {
        // 点击头像
    ...
    updataReward();
    setInterval(updataReward, 1e3 * 60);
...
define("app/phone/run_sing/top60", [ "core/zepto/zepto", "util/vote/vote", "core/jquery/1.8.3/jquery", "util/cookie/cookie", "util/loader/loader", "util/log/log", "util/platform/plt", "util/browser/browser", "util/net/urlquery", "util/user/user", "client", "util/others/others", "core/underscore/1.8.3/underscore", "util/swipe/swiper.min", "util/Timer/timer", "util/vote/uniformDate", "util/ppsdk/sdkUtil", "util/log/alertBox", "util/ppsdk/sdkUtil", "util/ppsdk/share" ], function(require) {
    var $ = require("core/zepto/zepto");
    var vote = new (require("util/vote/vote"))();
    var timer = require("util/vote/vote");
    var Loader = require("util/loader/loader");
    var others = require("util/others/others");
    var sdk = require("util/ppsdk/sdkUtil");
    ...
    Loader.ajax({
        ...
        cache: true,
        success: function(data) {
            servertime = others.newDate(data * 1e3);
        },
        timeout: 1e3
    });
    if (!$module.length) {
        ...
        Loader.ajax({
            ...
            success: function(data) {
                ...
                if (data.code == 1) {
                    ...
                    //这里返回的是时间戳
                    var endTime = others.newDate(Number(data.data.end * 1e3));
                    ...
                    if (startTime > endTime) {
                        //后端返回的结束时间可能会小于当前时间，不加判断的话会循环走到status==2这个逻辑里
                        clearInterval(cycleTimer1);
                        $(".time").html("00:00:00");
                        if (!isEnd) {
                            setTimeout(function() {
...
define("app/phone/run_sing/race_vote", [ "core/jquery/1.8.3/jquery", "util/others/others", "util/cookie/cookie", "core/underscore/1.8.3/underscore", "util/swipe/swiper.min", "util/vote/vote", "util/loader/loader", "util/log/log", "util/platform/plt", "util/browser/browser", "util/net/urlquery", "util/user/user", "client", "util/Timer/timer", "util/vote/uniformDate", "util/ppsdk/sdkUtil", "util/log/alertBox", "util/ppsdk/sdkUtil" ], function(require) {
    var $ = require("core/jquery/1.8.3/jquery");
    var others = require("util/others/others");
    var Swiper = require("util/swipe/swiper.min");
...
define("app/phone/run_sing/vacant", [ "core/jquery/1.8.3/jquery", "util/loader/loader", "util/log/log", "util/platform/plt", "util/browser/browser", "util/net/urlquery", "util/others/others", "util/cookie/cookie", "core/underscore/1.8.3/underscore", "util/swipe/swiper.min", "util/vote/vote", "util/user/user", "client", "util/Timer/timer", "util/vote/uniformDate", "util/ppsdk/sdkUtil", "util/log/alertBox" ], function(require) {
        ...
        Loader.ajax({
            ...
            success: function(data) {
                ...
                if (Number(isEnd) === 0) {
                    ...
                    setTimeout(updateList, 3 * 60 * 1e3);
                } else {
                ...
                $module.find(".vacant-list").empty();
                // 清空列表
                listNew.forEach(function(v, i, a) {
                    // 渲染列表
                    ...
                    var $template = $("" + "<div is_virtual=" + is_virtual + " username=" + username + ' class="vacant-item' + isEmpty + " " + classIndex + '">' + '<div class="vacant-item-bg"></div>' + '<div class="vacant-avatar">' + '<img src="' + avatar + '" alt="">' + "</div>" + '<div class="vacant-item-name">' + name + "</div>" + "</div>");
    ...
    updateList();
    $module.on("click", ".vacant-item:not(.empty)", function(e) {
        // 点击头像打开主页
        var that = this;
...
define("app/phone/run_sing/score", [ "core/jquery/1.8.3/jquery", "util/loader/loader", "util/log/log", "util/platform/plt", "util/browser/browser", "util/net/urlquery", "util/others/others", "util/cookie/cookie", "core/underscore/1.8.3/underscore", "util/swipe/swiper.min", "util/vote/vote", "util/user/user", "client", "util/Timer/timer", "util/vote/uniformDate", "util/ppsdk/sdkUtil", "util/log/alertBox" ], function(require) {
    var $ = require("core/jquery/1.8.3/jquery");
    var Loader = require("util/loader/loader");
    var others = require("util/others/others");
    var sdk = require("util/ppsdk/sdkUtil");
    ...
    Loader.ajax({
        ...
        success: function(data) {
            ...
            listNew.forEach(function(v, i, a) {
                ...
                var is_virtual = v.player_isVirtual;
                var name = v.player;
                var $template = $("" + '<div class="score-item">' + "<div is_virtual=" + is_virtual + ' class="score-avatar" username=' + username + ">" + '<div class="score-bg"></div>' + '<img src="' + avatar + '" alt="">' + "</div>" + '<div class="score-name">' + name + "</div>" + '<div class="score-info">' + '<span class="score-key">前进: </span><span class="score-value">' + v.player_praise + "</span><br />" + '<span class="score-key">后退: </span><span class="score-value">' + v.player_hate + "</span><br />" + '<span class="score-key">时长: </span><span class="score-value">' + transferSecond(v.player_time) + "</span><br />" + "</div>" + "</div>");
                $template.appendTo($module.find(".score-list"));
            });
        }
    });
    $module.on("click", ".score-avatar", function(e) {
        // 点击头像打开主页
        var that = this;
        if ($(this).attr("is_virtual") === "1") {
            if (others.checkIsInApp()) {
